...
function writeFile(path, binary, callback) {
  mkdirp(nodePath.dirname(path), function (err) {
    if (err) {
      ...
      nodeFs.writeFile(path, binary, function (err) {
        if (err) {
          // XXX This is a hopefully temporary work-around for failing to
          // overwrite existing objects, when the native client drops them
